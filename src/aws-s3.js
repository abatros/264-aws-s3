...
function parse_s3filename(fn) {
  if (fn.startsWith('s3://')) fn = fn.substring(5);
  const v = fn.match(/([^\/]+)\/(.+)$/)
...
async function putObject(p1, body) {
  ...
  return new Promise((resolve, reject) =>{
    ...
    if (p1.Key.endsWith('.html')) {
      ...
      p1.ContentEncoding = p1.ContentEncoding || 'utf8';
    }

    if (p1.Key.endsWith('.css')) {
      p1.ACL = p1.ACL || 'public-read';
      p1.ContentType = 'text/css';
      p1.ContentEncoding = 'utf8';
    }

    if (p1.Key.endsWith('.js')) {
      p1.ACL = p1.ACL || 'public-read';
      p1.ContentType = 'text/javascript';
      p1.ContentEncoding = 'utf8';
    }
...
async function headObject(p1) {
  const verbose =1;
  ;(verbose >0) && console.log(`@617 p1:`,p1)

  if (typeof p1 == 'string') {
    console.log(`@76 p1:`,p1)
    p1 = parse_s3filename(p1)
    console.log(`@78 aws-s3:headObject p1:`,p1)
    if (!p1.Bucket) {
      console.log(stackTrace.get().toString())
      //throw 'fatal@620'
    }
  }
...
async function touch(p1) {
  const etime = new Date().getTime()
  return new Promise((resolve, reject) =>{
    if (typeof p1 == 'string') {
      p1 = parse_s3filename(p1) // Bucket,Key
    }
    if(!p1.Bucket) {
      reject(`touch_Object: Missing Bucket in `,p1)
    }
    if(!p1.Key) {
      reject(`touch_Object: Missing Key in `,p1)
    }
    ...
    p1 = {Bucket, Key, Body:'', ACL, ContentType, ContentEncoding};
